  ...
  return (
    <div style={{ background: "var(--papier)" }}>
      ...
      <header
        ...
        style={{ background: "var(--blauw-vlak)", borderRadius: "0 0 36px 36px", padding: "56px 0 48px" }}
      >
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <h1 className="font-black leading-tight tracking-tight" style={{ fontSize: "clamp(2rem, 5vw, 3.2rem)" }}>
            Over Derdeplekken.nl
          </h1>
          <p className="mt-4 text-lg max-w-2xl text-gray-700">
            Een platform voor iedereen die op zoek is naar een fijne plek buiten huis en werk. Gratis, voor iedereen, in elke stad.
          </p>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <section className="mt-14 grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <p className="mt-4 text-base text-gray-700 leading-relaxed">
              Veel mensen weten niet dat er vlakbij hun huis een bibliotheek is die tot 21:00 open is, of een park dat altijd toegankelijk is. Derdeplekken.nl maakt die plekken zichtbaar op een overzichtelijke, interactieve kaart.
            </p>
            <p className="mt-4 text-base text-gray-700 leading-relaxed">
              We richten ons op mensen die een rustige plek zoeken om te werken of studeren, bewoners die hun wijk beter willen leren kennen, en iedereen die op zoek is naar verbinding buiten de muren van huis of kantoor.
            </p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
            {categorieen.map((cat) => (
              <div key={cat.label} className="p-4 rounded-xl2 border-2 border-inkt flex flex-col gap-3" style={{ background: cat.bg }}>
                <span className="w-9 h-9 rounded-full border-2 border-inkt flex items-center justify-center" style={{ background: cat.color }} aria-hidden="true">
                  {cat.icon}
                </span>
                <div>
          ...
          <div className="mt-6">
            <Link href="/utrecht" className="block max-w-sm border-2 border-inkt rounded-xl2 overflow-hidden hover:-translate-y-0.5 transition-transform" style={{ background: "#fff" }}>
              <div className="px-5 py-4 flex items-center justify-between text-white" style={{ background: "#cc0000" }}>
                ...
                <span className="px-4 py-1.5 rounded-full text-sm font-extrabold" style={{ background: "#fff", color: "#cc0000" }}>
                  Bekijk kaart
                </span>
              </div>
            ...
            {tijdlijn.map((item, i) => (
              <div key={item.jaar} className="flex gap-5">
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-full border-2 border-inkt flex items-center justify-center font-black text-xs flex-shrink-0" style={{ background: i === tijdlijn.length - 1 ? "var(--koraal)" : "var(--blauw-vlak)" }}>
                    {item.jaar}
                  </div>
                  {i < tijdlijn.length - 1 && <div className="w-0.5 flex-1 my-1" style={{ background: "var(--inkt)", minHeight: 24 }} />}
                </div>
                <div className="pt-2 pb-6">
                  <p className="text-base text-gray-700">{item.tekst}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mt-10 mb-24 p-8 rounded-xl2 border-2 border-inkt" style={{ background: "var(--blauw-vlak)" }}>
          ...
          <p className="mt-3 text-base text-gray-700">
            Ken je een derde plek die nog niet op de kaart staat? Neem contact op, we horen het graag. Heb je een horecazaak en wil je openingstijden of een verblijfsdeal doorgeven? Gebruik dan het horecaportaal.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/portaal"
              className="inline-flex items-center gap-2 font-bold text-sm px-5 py-2.5 rounded-full"
              style={{ border: "2px solid var(--inkt)", background: "var(--geel-vlak)" }}
            >
              Portaal voor horeca <ArrowRight size={14} />
            </Link>
            <a
              href="mailto:hallo@derdeplekken.nl"
