...
const typeItems = [
  {
    ...
    label: "Bibliotheken",
    text: "Gratis, stil, met werkplekken en wifi. Ideaal om te studeren of te werken.",
  },
  {
    color: "#ff7f63",
    icon: <Users size={16} />,
    label: "Buurtcentra",
    text: "Ontmoeting, activiteiten en een kop koffie. Vaak gratis of heel goedkoop.",
  },
  {
    color: "#56ddac",
    icon: <TreePine size={16} />,
    label: "Parken",
    text: "Gratis, buiten, altijd open. Seizoensafhankelijk maar onmisbaar.",
  },
  {
    color: "#f9fa97",
    icon: <Coffee size={16} />,
    label: "Horeca (binnenkort)",
    text: "Cafés met verblijfsdeals tijdens rustige uren. Speciaal voor studenten.",
  },
];
      ...
      <div>
        ...
        <div className="space-y-3">
          {infoBlocks.map((block) => (
            <div
              key={block.title}
              className="p-4 rounded-xl2 border-2 border-inkt"
        ...
        <div className="space-y-2">
          {typeItems.map((item) => (
            <div key={item.label} className="flex items-start gap-3 p-3 rounded-xl bg-white border-2 border-inkt">
              <span
                className="w-7 h-7 rounded-full border-2 border-inkt flex items-center justify-center flex-shrink-0"
